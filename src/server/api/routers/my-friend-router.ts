...
const userMutualFriendCount = (db: Database, userId: number, friendUserId:number) => {
  ...
  return db
    ...
    .where('friendships.friendUserId', 'in', [userId, friendUserId])
    .select((eb) => [
      'friendships.userId',
      'friendships.friendUserId',
      eb.fn.count<number>('friendships.userId').distinct().as('mutualFriendCount'),
    ])
    .groupBy('friendships.userId')
    .having((eb) => eb.fn.count('friendships.friendUserId'), '>', 1)
}
