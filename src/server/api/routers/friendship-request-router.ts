...
export const friendshipRequestRouter = router({
  send: procedure
    ...
    .mutation(async ({ ctx, input }) => {
       ...
       */
      const checkExistingRequest = await ctx.db
          .selectFrom('friendships')
          .where('friendships.userId', '=', ctx.session.userId)
          .where('friendships.friendUserId', '=', input.friendUserId)
          .where('friendships.status', '=', FriendshipStatusSchema.Values['declined'])
          .select('userId')
          .execute();

      if(checkExistingRequest.length > 0){
        await ctx.db
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        ...
        await t
          ...
          .returning(['userId', 'friendUserId', 'status'])
          .execute();
        const checkExistingRequest = await t
          .selectFrom('friendships')
          .where('friendships.userId', '=', ctx.session.userId)
          .where('friendships.friendUserId', '=', input.friendUserId)
          .select('userId')
